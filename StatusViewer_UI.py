...
from JobStruct import connect_db, create_jobs_table, parse_job_data, parse_json_to_job_reason_pairs
from Main import DEFAULT_CONFIG_PATH, DEFAULT_RUN_TIMES, _get_next_run, _normalize_name_need, _parse_run_times
from PipelineStatus import STATUS_PATH, append_event, read_status, update_status
...
def load_config_users(config: Dict[str, Any]) -> Dict[str, Dict[str, str]]:
    return _normalize_name_need(config.get("nameNeed", {}))
...
def launch_manual_scrape(config_path: Path) -> tuple[bool, str]:
    ...
    command = (
        "from Main import _configure_logging, run_pipeline_once; "
        "from pathlib import Path; "
...
try:
    next_from_input = _get_next_run(datetime.now(), _parse_run_times(run_times_text)).strftime("%Y-%m-%d %H:%M:%S")
except Exception as exc:
