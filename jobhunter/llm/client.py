...
from jobhunter.scrapers.runner import run_enabled_scrapers
...
from jobhunter.observability.diagnostic_logger import get_logger, start_diagnostic_run, end_diagnostic_run
...
def FindMeSomeJobs(UserNeeds: list = ["ART in general"], UserNames: list = ["Thea"], jobFinder: LLMClient = None):
    ...
    scraper_specs = enabled_scrapers()
    update_status(
        state="running",
        phase="scraping",
        ...
        scrape_sources_done=[],
        scrape_sources_total=len(scraper_specs),
        llm_batches_done=0,
        llm_batches_total=0,
        db_writes_done=0,
        db_writes_total=0,
    ...
    scraper_results = []
    completed_scrapers = []
    for scraper in scraper_specs:
        update_status(phase="scraping", current_source=scraper.name)
        result = run_enabled_scrapers([scraper])[0]
        scraper_results.append(result)
        completed_scrapers.append(scraper.name)
        append_event("scraper_finished", source=scraper.name, job_count=len(result.jobs))
        update_status(scrape_sources_done=completed_scrapers, current_source="")
        _diag_logger.log_job_search_results(scraper.name, result.jobs)

    alljobs = [job for result in scraper_results for job in result.jobs] #all jobs!
    ...
    update_status(
        ...
        total_jobs_before_dedupe=(
            sum(len(result.jobs) for result in scraper_results)
        ),
    )
    ...
    _diag_logger.log_job_search_results(
        ...
        alljobs,
        extra={
            "total_before_dedupe": sum(len(result.jobs) for result in scraper_results)
        },
    )
